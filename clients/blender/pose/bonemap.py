...
def rotatePoint(point, center, angle):
    ...
    rotatedX = (
        math.cos(angle) * (point[0] - center[0])
        - math.sin(angle) * (point[1] - center[1])
        + center[0]
    )
    rotatedY = (
...
class PersonJSONData:
    def __init__(
        self, pose, face, EarLobeToEarLobedistanceInBlenderUnits, TieEyelidsTogether
    ):
        self.poseStart = pose
        self.faceStart = face
        ...
        self.EarLobeToEarLobedistanceInBlenderUnits = (
            EarLobeToEarLobedistanceInBlenderUnits
        )
    ...
    def GetCurrentPointHeadTiltCorrected(self, Point):
        ...
        PointCorrrected = rotatePoint(
            GetPoint(self.faceCurrent, 8), Point, (angle * -1)
        )
    ...
    def ConvertPixelPointToBlenderUnits(self, pnt):
        pntx = pnt[0] * (
            self.EarLobeToEarLobedistanceInBlenderUnits / self.distancBetweenEars
        )
        pnty = pnt[1] * (
            self.EarLobeToEarLobedistanceInBlenderUnits / self.distancBetweenEars
        )
    ...
    def getHeadTiltSideToSide(self):
        ...
        if leftEar[2] < 0.1 or rightEar[2] < 0.1:
    ...
    def getJawTiltUpDown(self):
        ...
        # Then we
        NoseToJawStartDist = self.GetDistanceBetweenTwoCurrentPoints(
            GetPoint(self.faceStart, 8), GetPoint(self.faceStart, 30)
        )
        NoseToJawCurrentDist = self.GetDistanceBetweenTwoCurrentPoints(
            GetPoint(self.faceCurrent, 8), GetPoint(self.faceCurrent, 30)
        )
        ...
        PivottoEndOfJaw = self.distancBetweenEars - 0.125 * self.distancBetweenEars
    ...
    def getFacePosition(self, ptnum1, ptnum2, RelativeTo):
        ptAvgStart = AverageTwoPoints(
            GetPoint(self.faceStart, ptnum1), GetPoint(self.faceStart, ptnum2)
        )
        ptAvgCurrent = AverageTwoPoints(
            GetPoint(self.faceCurrent, ptnum1), GetPoint(self.faceCurrent, ptnum2)
        )

        if RelativeTo == "LEFTEYE":
            ptDiffCurrent = DifferenceBetweenPoint(
                ptAvgCurrent, GetPoint(self.poseCurrent, 16)
            )
            ...
            ptDiffCurrent = DifferenceBetweenPoint(
                ptAvgCurrent, GetPoint(self.poseCurrent, 15)
            )
            ...
            ptDiffCurrent = DifferenceBetweenPoint(
                ptAvgCurrent, GetPoint(self.faceCurrent, 8)
            )
            ptDiffStart = DifferenceBetweenPoint(ptAvgStart, self.StartFaceChinPosition)
        elif RelativeTo == "upperlip":
            ptDiffCurrent = DifferenceBetweenPoint(
                ptAvgCurrent, GetPoint(self.faceCurrent, 51)
            )
            ptDiffStart = DifferenceBetweenPoint(ptAvgStart, self.StartFaceChinPosition)
        else:
            ptDiffCurrent = DifferenceBetweenPoint(
                ptAvgCurrent, GetPoint(self.faceCurrent, 30)
            )
    ...
    #     else:
    #         thisref = ref
    #     output = self.ConvertPixelPointToBlenderUnits(
    #         self.getFacePosition(8, 8, thisref)
    #     )
    #     return multiply(output, correction)

    def getEyeTrackLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(69, 69, thisref)
        )
        return multiply(output, correction)

    def getEyeTrackRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(68, 68, thisref)
        )
        return multiply(output, correction)

    def getUpperLidLeftUp(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(43, 44, thisref)
        )
        return multiply(output, correction)

    def getUpperLidRightUp(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(37, 38, thisref)
        )
        return multiply(output, correction)

    def getUpperLidDownLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(47, 46, thisref)
        )
        return multiply(output, correction)

    def getUpperLidDownRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(41, 40, thisref)
        )
        return multiply(output, correction)

    def getNostrilLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(35, 35, thisref)
        )
        return multiply(output, correction)

    def getNostrilRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(31, 31, thisref)
        )
        return multiply(output, correction)

    # match
    def getLipUpperLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(52, 63, thisref)
        )
        return multiply(output, correction)

    # match
    def getLipUpperRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(50, 61, thisref)
        )
        return multiply(output, correction)

    def getLipUpperCenter(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(
                57, 67, thisref
            )  # 51,52,53,61,62,63 - few more landmarks to average of
        )
        return multiply(output, correction)

    def getLipLowerCenter(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(61, 61, thisref)
        )
        return multiply(
            output, correction
        )  # 58,57,56,67,66,65 - few more landmarks to average of

    def getLipLowerRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(59, 67, thisref)
        )
        return multiply(output, correction)

    def getLipLowerLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(55, 65, thisref)
        )
        return multiply(output, correction)

    # match
    def getLipStretchLeft(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(54, 64, thisref)
        )
        return multiply(output, correction)

    # match
    def getLipStretchRight(self, correction, ref):
        if ref == "DEFAULT":
            thisref = "NOSE"
        else:
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(48, 60, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyeBrowLeftOuter(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(25, 26, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyeBrowLeftInner(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(23, 24, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyeBrowRightInner(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(19, 20, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyeBrowRightOuter(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(17, 18, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyelidLowerRight(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(40, 41, thisref)
        )
        return multiply(output, correction)
    ...
    def getEyelidLowerLeft(self, correction, ref):
        if ref == "DEFAULT":
            ...
            thisref = ref
        output = self.ConvertPixelPointToBlenderUnits(
            self.getFacePosition(46, 47, thisref)
        )
        return multiply(output, correction)
...
class BoneJSONData:
    def __init__(
        SourceBoneLocationName,
        DestinationBoneName,
        BoneModificationType,
        ApplyToX,
        ApplyToY,
        DirectionQuatToApplyXandY,
    ):
...
class BoneToMap:
    ...
    def KeyFrame(self):
        ...
        if BoneModificationType == "ROT":
            self.Bone.keyframe_insert(
                data_path="rotation_euler", frame=FinalFrameNumber
            )
...
class ReadInApplyToRigOperator(bpy.types.Operator):
    ...
    def get_correction_Angle(self, bone_axis, correction):
        ...
        if bone_axis == "PLUSX" or bone_axis == "NEGX":
            cAngle = mathutils.Euler(
                (math.radians(correction), math.radians(0), math.radians(0)), "XYZ"
            ).to_quaternion()
        if bone_axis == "PLUSY" or bone_axis == "NEGY":
            cAngle = mathutils.Euler(
                (math.radians(0), math.radians(correction), math.radians(0)), "XYZ"
            ).to_quaternion()
        if bone_axis == "PLUSZ" or bone_axis == "NEGZ":
            cAngle = mathutils.Euler(
                (math.radians(0), math.radians(0), math.radians(correction)), "XYZ"
            ).to_quaternion()
        return CAngle
    ...
    def execute(self, context):
        ...
        while keepGoing:
            ...
            mypath = bpy.path.abspath(
                filename[0:beforeNumber] + numberString + "_keypoints.json"
            )
            ...
            if path.exists(mypath):
                ...
                if len(people) == 1:
                    ...
                    print(
                        "Warning: More than one person found in file... finding person with non Zero Face Points"
                    )
                ...
                if peopleFound:
                    ...
                    if FirstTime:
                        p1 = PersonJSONData(
                            pose,
                            face,
                            op2rig.ear_to_ear_conversion_distance,
                            op2rig.tie_eyelids_together,
                        )
                        FirstTime = False
                    else:
                        ...
                        if bone_settings.SourceBoneType == "FACE":
                            if bone_settings.BoneModificationType == "ROT":
                                if bone_settings.SourceBoneLocationNameFace == "head":
                                    ...
                                    print(
                                        "Bone: "
                                        + bone_settings.SourceBoneLocationNameFace
                                        + " not supported in rotation mode."
                                    )
                                ...
                                if apply:
                                    if bone_settings.ApplyToX:
                                        if bone_settings.BoneHorizontalAxis == "PLUSX":
                                            ...
                                            bone_settings.BoneHorizontalAxis == "PLUSY"
                                        ):
                                            y = horizontal
                                        elif bone_settings.BoneHorizontalAxis == "NEGY":
                                            y = horizontal * -1
                                        elif (
                                            bone_settings.BoneHorizontalAxis == "PLUSZ"
                                        ):
                                            z = horizontal
                            ...
                            if bone_settings.BoneModificationType == "LOC":
                                ...
                                if (
                                    ...
                                    if (
                                        not eyelidtiedtoegetherfound
                                        and op2rig.tie_eyelids_together
                                    ):
                                    ...
                                    print(
                                        "Bone: "
                                        + bone_settings.SourceBoneLocationNameFace
                                        + " not supported in location mode."
                                    )
                                if apply:
                                    horizontal = Offset[0]
                                    vertical = Offset[1]
                                    if bone_settings.ApplyToX:
                                        if bone_settings.BoneHorizontalAxis == "PLUSX":
                                            ...
                                            z = horizontal * -1
                                        if (
                                            bone_settings.ApplyTranslationAlongMoreAxisHorizontal
                                        ):
                                            if (
                                                (
                                                    bone_settings.BoneHorizontalAxis
                                                    == "PLUSX"
                                                    or bone_settings.BoneHorizontalAxis
                                                    == "NEGX"
                                                )
                                                and bone_settings.TranslationAlongMoreAxisHorizontal.x
                                                != 0
                                            ):
                                                y = y + x * (
                                                    bone_settings.TranslationAlongMoreAxisHorizontal.y
                                                    / bone_settings.TranslationAlongMoreAxisHorizontal.x
                                                )
                                                z = z + x * (
                                                    bone_settings.TranslationAlongMoreAxisHorizontal.z
                                                    / bone_settings.TranslationAlongMoreAxisHorizontal.x
                                                )
                                            if (
                                                (
                                                ...
                                                and bone_settings.TranslationAlongMoreAxisHorizontal.y
                                                != 0
                                            ):
                                                x = x + y * (
                                                    bone_settings.TranslationAlongMoreAxisHorizontal.x
                                            ...
                                            if (
                                                ...
                                                and bone_settings.TranslationAlongMoreAxisHorizontal.z
                                                != 0
                                            ):
                                                x = x + z * (
                                                    bone_settings.TranslationAlongMoreAxisHorizontal.x
                                    ...
                                    if bone_settings.ApplyToY:
                                        if bone_settings.BoneVerticalAxis == "PLUSX":
                                            ...
                                            z = vertical * -1
                                        if (
                                            bone_settings.ApplyTranslationAlongMoreAxisVertical
                                        ):
                                            if (
                                                (
                                                    bone_settings.BoneVerticalAxis
                                                    == "PLUSX"
                                                    or bone_settings.BoneVerticalAxis
                                                    == "NEGX"
                                                )
                                                and bone_settings.TranslationAlongMoreAxisVertical.x
                                                != 0
                                            ):
                                                y = y + x * (
                                                    bone_settings.TranslationAlongMoreAxisVertical.y
                                                    / bone_settings.TranslationAlongMoreAxisVertical.x
                                                )
                                                z = z + x * (
                                                    bone_settings.TranslationAlongMoreAxisVertical.z
                                                    / bone_settings.TranslationAlongMoreAxisVertical.x
                                                )
                                            if (
                                                (
                                                ...
                                                and bone_settings.TranslationAlongMoreAxisVertical.y
                                                != 0
                                            ):
                                                x = x + y * (
                                                    bone_settings.TranslationAlongMoreAxisVertical.x
                                            ...
                                            if (
                                                ...
                                                and bone_settings.TranslationAlongMoreAxisVertical.z
                                                != 0
                                            ):
                                                x = x + z * (
                                                    bone_settings.TranslationAlongMoreAxisVertical.x
                    ...
                    for bone_settings in bone_list:
                        ...
                        if bone_settings.UseCustomKeyFrameNumber:
                            if (
                                CurrentFrame % bone_settings.CustomBonKeyFrameNumber
                                == 0
                            ):
                                if bone_settings.BoneModificationType == "ROT":
                                    bone.rotation_mode = "XYZ"
                                    bone.keyframe_insert(
                                        ...
                                        frame=FinalFrameNumber,
                                    )
                                if bone_settings.BoneModificationType == "LOC":
                                    bone.keyframe_insert(
                                        data_path="location", frame=FinalFrameNumber
                                    )
                            ...
                            if not eyelidfastmotiondetected:
                                ...
                                AmountTheEyelidChanged = abs(
                                    eyelid_l_current - PreviousEyelidValue_l
                                )
                                if (
                                    AmountTheEyelidChanged
                                    > op2rig.eyelid_noise_removal_distance
                                ):
                                    eyelidfastmotiondetected = True
                            if eyelidfastmotiondetected or (
                                CurrentFrame % op2rig.eye_keyframe_every_n_frames == 0
                            ):
                                bone.keyframe_insert(
                                    data_path="location", frame=FinalFrameNumber
                                )
                            ...
                            if not eyelidfastmotiondetected:
                                # keyfram if the eyelid changes a LOT. That way we catch blinks.
                                AmountTheEyelidChanged = abs(
                                    eyelid_r_current - PreviousEyelidValue_r
                                )
                                if (
                                    AmountTheEyelidChanged
                                    > op2rig.eyelid_noise_removal_distance
